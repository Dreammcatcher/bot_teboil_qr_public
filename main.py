    ...
    if (user_id in blacklist_name) or (user_name in blacklist_name.values()):
        ...
        if num_kart is not None:
            ...
            if (status_sell[0]) is None:
                ...
                await bot.send_message(message.chat.id, '👉Подождите, выдается карта лояльности...')
                await bot.send_message(message.chat.id, '⚠️ Номер дампа  будет автоматически удален через - 3 часа')
                balance = session.query(Teboil.balans).filter_by(code=enters_code).first()[0]
                num_kart = session.query(Teboil.num_kart).filter_by(code=enters_code).first()[0]
                logging.info(f'Введен правильный код - {enters_code}, пользователь - {user_info}')

                num_k = await bot.send_message(message.chat.id, f'Ваш дамп - {num_kart}\n🟩 Баллов на карте - {balance}')
                ...
                if time_del_kart != '':
                    ...
                    asyncio.create_task(delete_message(num_k, int(time_del_kart)))
            else:
